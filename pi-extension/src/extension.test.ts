...
describe("agent-network mesh delivery", () => {
  ...
  test("appends an idle FIFO batch once before one separate hidden follow-up wake", async () => {
    ...
    _deliverMeshMessageToAgentForTest(envelope("mesh-message-2"));
    ...
    expect(meshAppends(sendMessage).map(([message]) => (message as { content: string }).content)).toEqual([
      expect.stringContaining("mesh-message-1"),
      expect.stringContaining("mesh-message-2"),
    ]);
  ...
  test("shutdown invalidates stale settle work and a replacement drains new mesh work", async () => {
    ...
    _setDisposedForTest(false);
    let replacementIdle = true;
    startSession(harness, () => replacementIdle);
...
describe("ask_user prompt forwarding", () => {
  ...
  test("rejected Android answer remains retryable until an authoritative CLI answer resolves it", async () => {
    ...
    expect(sender.send).toHaveBeenCalledWith(expect.objectContaining({
      ...
      message: expect.stringContaining("response was not accepted locally"),
    }));
    expect(sender.send).not.toHaveBeenCalledWith(expect.objectContaining({
    ...
    const lateSender = { send: vi.fn() };
    _routeClientMessageFrom(lateSender as unknown as { send: ReturnType<typeof vi.fn> }, {
      type: "ask_user_response",
      id: "tc_ask_race",
      response: { kind: "selection", selections: ["A"] },
    }, { abort: vi.fn() });
    expect(lateSender.send).toHaveBeenCalledWith(expect.objectContaining({
      message: "No pending ask_user prompt for id tc_ask_race",
    }));
  });
  ...
  test("session_new clears stale pending ask_user prompts", async () => {
    await _pairForTest("peer-ask-new-session");

    const harness = captureEventHarness();
    const responder = vi.fn();
    harness.emitBus("ask:prompt", {
      toolCallId: "tc_ask_new_session",
      question: "Old prompt?",
      respond: responder,
    });
    await new Promise<void>((r) => setImmediate(r));
    const sentTypes = relayRef.current!.send.mock.calls
      .map((c) => decodeSentCt(c[0] as string).inner.type);
    expect(sentTypes).toContain("ask_user_prompt");

    const remotePi = captureHandler("remote-pi");
    const ctx = {
      ...makeMockCtx(),
      newSession: vi.fn(async (opts?: { withSession?: (ctx: unknown) => Promise<void> }) => {
        await opts?.withSession?.(ctx);
        return { cancelled: false };
      }),
    };
    await remotePi("status", ctx);

    routeClientMessage(
      { type: "session_new", id: "new-ask-session" },
      { abort: vi.fn() },
    );
    await vi.waitFor(() => {
      const sent = relayRef.current!.send.mock.calls
      ...
      expect(sent.some((m) => (
        m.type === "session_history" && m.in_reply_to === "new-ask-session"
      ))).toBe(true);
    });

    const sender = { send: vi.fn() };
    _routeClientMessageFrom(
      sender as unknown as { send: ReturnType<typeof vi.fn> },
      {
        type: "ask_user_response",
        id: "tc_ask_new_session",
        response: { kind: "selection", selections: ["Done"] },
      },
      { abort: vi.fn() },
    );
    ...
    expect(sender.send).toHaveBeenCalledWith(expect.objectContaining({
      type: "error",
      code: "invalid_message",
      message: expect.stringContaining("No pending ask_user prompt"),
    }));
  ...
  test("reconnected v2 owner receives resolved ask_user when only the toolResult is replayable", async () => {
    await _pairForTest("peer-ask-replay-resolved");
    _setSessionStartedAtForTest(1);
    _setMessageBufferForTest([
      {
        role: "ask_user_prompt",
        toolCallId: "tc_ask_replay_resolved",
        timestamp: 10,
        askUser: {
          question: "Did the bad plan file get created successfully?",
          options: [{ title: "Done" }],
          allowMultiple: false,
          allowFreeform: true,
          allowComment: true,
      ...
      },
      {
        ...
        toolCallId: "tc_ask_replay_resolved",
        toolName: "ask_user",
        content: [{ type: "text", text: "User answered: You create the file. Then let me know." }],
        isError: false,
        timestamp: 11,
      },
    ]);
    ...
    const sendsBefore = relayRef.current!.send.mock.calls.length;
    relayRef.current!.emit("message", JSON.stringify({
      peer: "peer-ask-replay-resolved",
      ct: Buffer.from(JSON.stringify({
        type: "session_sync",
        id: "sync-ask-replay-resolved",
        capabilities: ["ask_user_prompt_cards_v2"],
      })).toString("base64"),
    }));
    await new Promise<void>((r) => setImmediate(r));

    const history = relayRef.current!.send.mock.calls
      .slice(sendsBefore)
      .map((c) => c[0] as string)
      .map(decodeSentCt)
      .find((d) => d.inner.type === "session_history" && d.inner.in_reply_to === "sync-ask-replay-resolved")!;
    const events = history.inner["events"] as Array<Record<string, unknown>>;
    expect(events.map((e) => e.type)).toEqual(["ask_user_prompt", "ask_user_resolved"]);
    expect(events[1]).toMatchObject({
      type: "ask_user_resolved",
      id: "tc_ask_replay_resolved",
      answer_label: "You create the file. Then let me know.",
      cancelled: false,
    });
  });
});
...
describe("session sync", () => {
  ...
  test("mapping (regression): ask_user toolResult resolves replayed prompt when explicit marker is missing", () => {
    const events = _mapAgentMessagesToEvents([
      {
        role: "ask_user_prompt",
        toolCallId: "tc_ask_stale",
        timestamp: 10,
        askUser: {
          question: "Did the bad plan file get created successfully?",
          context: "ctx",
  ...
  test("mapping (regression): ask_user toolResult does not duplicate an explicit resolved marker", () => {
    const events = _mapAgentMessagesToEvents([
      {
        role: "ask_user_prompt",
        toolCallId: "tc_ask_once",
        timestamp: 10,
        askUser: {
          question: "Pick?",
          options: [{ title: "A" }],
    ...
    expect(events.filter((e) => e.type === "ask_user_resolved")).toHaveLength(1);
    expect(events).toHaveLength(2);
  });

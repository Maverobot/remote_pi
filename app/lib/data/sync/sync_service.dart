...
class SyncService extends Service {
  ...
  Future<void> respondAskUser(AskUserResponse response) async {
    // The visible prompt belongs to this service's active session, which can
    // briefly differ from the transport's mutable destination room. Rebind the
    // manager (and therefore its live channel) before sending the answer.
  ...
  bool _isRetryableAskUserError(String message) =>
      message.contains('ask_user prompt') &&
      message.contains('response was not accepted locally') &&
  ...
  void _onServerMessage(ServerMessage msg, [String? originEpk]) {
    ...
    switch (msg) {
      ...
      case ErrorMessage(:final inReplyTo, :final code, :final message):
        ...
        if (!_isRetryableAskUserError(message)) {
          _discardStreamingState();
          _clearSteeringLabels();
          _setWorking(false);
        }
